...
namespace ERP.Forms.Proveedores
{
    public partial class frmListado : FormBase
    {
        ...
        private void ConsultarDatos()
        {
            ...
            dgvDatos.SetDataSource(from a in ProveedoresRepository.ObtenerProveedores()
                                   orderby a.Id
                                   select new
                                   {
                                       a.Id,
        ...
        {
            var a = ObtenerProveedoresSeleccionado();
            if (MessageBox.Show("¿Está seguro de que desea eliminar el Proveedor seleccionado?",
                "Eliminar Proveedores", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                try
                {
                    ProveedoresRepository.Eliminar(a.Id);
                    ConsultarDatos();
                    dgvDatos.SetRow(r => Convert.ToDecimal(r.Cells[0].Value) == a.Id);
                }
                catch (Exception ex)
